# Baseline 与人工处置数据设计（#5，仅设计）

> 状态：**设计文档，尚未实现**。本切片只定义数据契约与校准流程，不落地任何数据存储或统计逻辑——因为在真实 CI 产出 `changed`/`evidence` 数据之前就建模，会基于不存在的信号拟合。实现留待真实扫描数据积累后单独切片。
...
**校准指标只读、离线计算**，不反向修改分析逻辑——先观察再调整规则（如 `HIGH_CONFIDENCE` 集、测试路径降级启发式）。
...
- 任何存储实现（数据库/文件）与统计/指标计算。
- 处置 UI 或采集工具。
- 风险评分数值模型。

## 触发条件

当真实 CI 产出稳定的 `changed`/`evidence` 数据、且积累了可处置的 finding 样本后，再开启实现切片。
